...
# Source: https://github.com/maxvfischer/keras-image-segmentation-loss-functions


def binary_tversky_coef(
    y_true: tf.Tensor, y_pred: tf.Tensor, beta: float, smooth: float = 1.0
) -> tf.Tensor:
    """
...
def binary_dice_coef_loss(
    ...
    def loss(y_true: tf.Tensor, y_pred: tf.Tensor) -> tf.Tensor:
        ...
        """
        return 1 - binary_tversky_coef(
            y_true=y_true, y_pred=y_pred, beta=0.5, smooth=smooth
        )
...
def binary_weighted_dice_crossentropy_loss(
    ...
    def loss(y_true: tf.Tensor, y_pred: tf.Tensor) -> tf.Tensor:
        ...
        dice_coefficient = binary_tversky_coef(
            y_true=y_true, y_pred=y_pred, beta=0.5, smooth=smooth
        )
...
def binary_weighted_cross_entropy(
    beta: float, is_logits: bool = False
) -> Callable[[tf.Tensor, tf.Tensor], tf.Tensor]:
    """
    ...
    def loss(y_true: tf.Tensor, y_pred: tf.Tensor) -> tf.Tensor:
        ...
        wce_loss = tf.nn.weighted_cross_entropy_with_logits(
            labels=y_true, logits=y_pred, pos_weight=beta
        )
...
def binary_balanced_cross_entropy(
    beta: float, is_logits: bool = False
) -> Callable[[tf.Tensor, tf.Tensor], tf.Tensor]:
    """
    ...
    """
    if beta == 1.0:  # To avoid division by zero
...
def binary_focal_loss(
    ...
    def loss(y_true: tf.Tensor, y_pred: tf.Tensor) -> tf.Tensor:
        ...
        """
        f_loss = (
            beta * (1 - y_pred) ** gamma * y_true * K.log(y_pred)
        )  # β*(1-p̂)ᵞ*p*log(p̂)
        f_loss += (
            (1 - beta) * y_pred**gamma * (1 - y_true) * K.log(1 - y_pred)
